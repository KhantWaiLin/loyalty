    ...
    <div className="relative flex flex-col w-full overflow-scroll transaction-history-wrapper no-scrollbar">
      <header className="flex flex-col z-30 bg-indigo-700 basis-2/12 ps-[20px] pr-[42%]">
        <section className="flex justify-between mt-[30px]">
          <div className="flex flex-col items-start justify-start w-6 h-6">
            <svg
              onClick={handleBack}
      ...
      <main className=" z-50 bg-white absolute left-0 rounded-t-2xl top-20 w-full h-[485px] basis-10/12 overflow-auto no-scrollbar pt-3">
          ...
          <PointList/>
          <PointList/>
          <div className="text-gray-400 text-[13px] font-normal  leading-tight mb-1 text-center">No More Result!</div>
      </main>
      {/* point history list */}
    </div>
  );
...
{
  /* <div className="flex flex-col w-full px-6 py-4 overflow-scroll transaction-history-wrapper">
  ...
  <table className="w-full border border-collapse border-gray-500">
    <thead>
      <tr>
        <th className="p-2 border border-gray-500">Point</th>
        <th className="p-2 border border-gray-500">Collected Type</th>
        <th className="p-2 border border-gray-500">Is In</th>
    ...
    <tbody>
      {data?.map((item) => {
        ...
        return (
          <tr key={item?.date}>
            <td className="p-2 border border-gray-500">{item?.point}</td>
            ...
            <td className="p-2 border border-gray-500">{formattedDate}</td>
          </tr>
        );
      })}
    </tbody>
